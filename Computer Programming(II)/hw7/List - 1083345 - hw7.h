...
public:
      ...
      : myData()
   {
       ...
       {
           push_back(right.myData.myHead->next->myVal);
           right.myData.myHead->next = right.myData.myHead->next->next;
       }
       right.myData.myHead->next = s;*/

       myData.myHead = new node;
       myData.myHead->myVal = Ty();
       myData.myHead->next = myData.myHead->prev = myData.myHead;

       nodePtr rightOriginal = right.myData.myHead->next;
       for (size_t i = 0; i < right.myData.mySize; ++i)
       {
           nodePtr newNode = new node;
           newNode->myVal = right.myData.myHead->next->myVal;
       ...
       right.myData.myHead->next = rightOriginal;
   }
   ...
   list& operator=( const list &right )
   {
       if (this != &right)
       {
           ...
           nodePtr sI = myData.myHead->next;
           nodePtr sII = right.myData.myHead->next;
           for (int i = 0; i < right.myData.mySize; ++i)
           {
               myData.myHead->next->myVal = right.myData.myHead->next->myVal;
               myData.myHead->next = myData.myHead->next->next;
               right.myData.myHead->next = right.myData.myHead->next->next;
           }
           myData.myHead->next = sI;
           right.myData.myHead->next = sII;*/
          ...
          if (myData.mySize > right.myData.mySize)
           ...
           nodePtr original = myData.myHead->next;
           nodePtr rightOriginal = right.myData.myHead->next;
   ...
   void push_front( const Ty &val )
   {
       ...
       newN->myVal = val;
       newN->prev = myData.myHead;
       newN->next = myData.myHead->next;
       myData.myHead->next->prev = newN;
       myData.myHead->next = newN;
       ++myData.mySize;
   }
   ...
   void push_back( const Ty &val )
   {
       nodePtr newN = new node;
       newN->myVal = val;
       newN->prev = myData.myHead->prev;
       newN->next = myData.myHead;
       myData.myHead->prev->next = newN;
       myData.myHead->prev = newN;
       ++myData.mySize;
   }
...
bool operator==( const list< Ty > &left, const list< Ty > &right )
{
    if (left.size() != right.size())
        return false;
    ListConstIterator<ListVal<Ty>> sI = left.begin();
    ListConstIterator<ListVal<Ty>> sII = right.begin();
    for (; sI != left.end(); ++sI, ++sII)
        if (*sI != *sII)
            return false;
    return true;
}
