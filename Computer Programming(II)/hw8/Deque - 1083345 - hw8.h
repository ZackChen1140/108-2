...
public:
   ...
   void push_front( const Ty &val )
   {
       ...
       {
           size_t newOff = (myData.myOff - 1) % (myData.mapSize * 4);
           if (newOff % 4 == 3 && myData.mySize >= 4 * (myData.mapSize - 1))
           {
               doubleMapSize();
               newOff = (myData.myOff - 1) % (myData.mapSize * 4);
           }

           if(newOff%4==3)//請記得，若省略會拖慢執行速度。
               if (myData.map[newOff / 4] == nullptr)
                   myData.map[newOff / 4] = new value_type[4]();

           myData.map[newOff / 4][newOff % 4] = val;
           myData.myOff = newOff;
       }
       ++myData.mySize;
   }
   ...
   void push_back( const Ty &val )
   {
       ...
       {
           value_type newBack = (myData.myOff + myData.mySize) % (myData.mapSize * 4);
...
//      return ( ( myData.off / 4 ) & ( myData.mapSize - 1 ) );
   ...
   void doubleMapSize()
   {
       if (myData.mapSize > 0)
       {
           value_type** newMap = new value_type * [2 * myData.mapSize]();

           for (size_t i = myData.myOff / 4; i < (myData.myOff + myData.mySize) / 4; ++i)
               newMap[i] = new value_type[4]();
           if ((myData.myOff + myData.mySize) % 4 != 0)
               newMap[(myData.myOff + myData.mySize) / 4] = new value_type[4]();

           for (size_t i = myData.myOff, j = myData.myOff; i < myData.myOff + myData.mySize; ++i, ++j)
           {
