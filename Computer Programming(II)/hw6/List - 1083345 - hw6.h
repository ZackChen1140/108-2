...
{
    ListNode<Ty> *sI = left.myHead->next;
    ListNode<Ty> *sII = right.myHead->next;
    int v = 0;
    for (int i = 0; i < right.mySize; ++i)
    {
        if (left.myHead->next != right.myHead->next)
            v = 1;
        left.myHead->next = left.myHead->next->next;
        right.myHead->next = right.myHead->next->next;
    }
    left.myHead->next = sI;
    right.myHead->next = sII;
    if (v)
        return false;
    else
        return true;
