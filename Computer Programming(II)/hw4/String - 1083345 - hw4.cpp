...
{
    myData.mySize = count;
    if (count > myData.myRes) resize(count);
   ...
   : myData()
{
    ...
    if (right.myData.mySize > myData.myRes) resize(right.myData.mySize);
    for (int i = 0; i < myData.mySize; i++)
...
void string::resize( const size_type newSize, const char ch )
{
    ...
    {
        size_type myResII = myData.myRes * 1.5;
