...
vector::vector( const vector &right )
{
    ...
    else
    {
        ...
        int* str = new int[rightSize];
        for (int i = 0; i < rightSize; i++)
            str[i] = *(right.myFirst + i);
        myFirst = str;
        myLast = myEnd = str + rightSize;
    }
}
...
vector& vector::assign( const vector &right )
{
   if( this != &right ) // avoid self-assignment
   {
       ...
       int capacityII = myEnd - myFirst;
       int OPFxCapacityII = 1.5 * capacityII;
       if (myFirst == nullptr)
       {
           int* str = new int[rightSize];
           for (int i = 0; i < rightSize; i++)
       ...
       {
           if (rightSize <= OPFxCapacityII)
           {
               int* str = new int[OPFxCapacityII];
               for (int i = 0; i < rightSize; i++)
                   str[i] = *(right.myFirst + i);
               myFirst = str;
               myLast = str + rightSize;
               myEnd = str + OPFxCapacityII;
           }
           else
           {
               int* str = new int[rightSize];
...
{
    int sizeII = myLast - myFirst;
    int capacityII = myEnd - myFirst;
    int OPFxCapacityII = capacityII * 1.5;
    if (newSize <= capacityII)
    {
        for (int i = newSize; i < capacityII; i++)
            *(myFirst + i) = 0;
        myLast = myFirst+newSize;
    }
    else if (newSize <= OPFxCapacityII)
    {
        int* str = new int[OPFxCapacityII];
        for (int i = 0; i < sizeII; i++)
            str[i] = *(myFirst + i);
        for (int i = sizeII; i < OPFxCapacityII; i++)
            str[i] = 0;
        clear();
        myFirst = str;
        myLast = str + newSize;
    ...
    else
    {
        ...
        for (int i = 0; i < sizeII; i++)
            str[i] = *(myFirst + i);
        for (int i = sizeII; i < newSize; i++)
            str[i] = 0;
        clear();
        myFirst = str;
        myLast = myEnd = str + newSize;
    }
}
